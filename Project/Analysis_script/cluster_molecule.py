...
def rmsd_obabel(conformation_1: str, conformation_2: str) -> float:
    ...
    for line in proc.stdout.splitlines():
        try:
            ...
            continue
    raise ValueError(
        f"RMSD not found in obabel output for {conformation_1.name} vs {conformation_2.name}\n{proc.stdout}"
    )
...
def main() -> None:
    ...
    args = p.parse_args()
    if args.output_dir is None and args.source_dir:
        args.output_dir = os.path.join(args.source_dir, "clusters")
