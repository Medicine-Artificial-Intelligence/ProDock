    ...
    for (s, m), d in DATA.items():
        ...
        for r in d["individual_results"]:
            row = {
                ...
                "train_delta": r.get("roc-auc_improvement")
                or r.get("roc_auc_improvement")
                or r.get("pr-auc_improvement")
                or r.get("logauc_improvement")
                or np.nan,
            }
            rows.append(row)
...
def fig_grouped_bars():
    ...
    for ax, m in zip(axes, METRICS):
        ...
        bars_b = ax.bar(
            ...
            bases,
            w,
            label="Baseline (rank-1)",
            color=[LIGHTER[s] for s in SCORING],
            edgecolor="white",
            linewidth=0.5,
        )
        bars_o = ax.bar(
            x + w / 2, opts, w, label="Optimised", color=[PALETTE[s] for s in SCORING], edgecolor="white", linewidth=0.5
        )

        # Value labels
        for bar in list(bars_b) + list(bars_o):
            h = bar.get_height()
            ax.text(bar.get_x() + bar.get_width() / 2, h + 0.005, f"{h:.3f}", ha="center", va="bottom", fontsize=6.5)
        ...
        fig.legend(handles=legend_patches, loc="upper center", bbox_to_anchor=(0.5, 0.95), ncol=2, frameon=False)
...
def fig_scatter_grid():
    ...
    for row_i, m in enumerate(METRICS):
        for col_i, s in enumerate(SCORING):
            ...
            n_imp = improved.sum()
            ax.text(
                0.05, 0.95, f"r = {r_val:.2f}\nn↑ = {n_imp}/{len(bases)}", transform=ax.transAxes, fontsize=7, va="top"
            )
...
def fig_delta_heatmap():
    ...
    ax.set_title(
        "Per-Protein Test-Set Δ (Optimised − Baseline)\nfor Each Scoring × Metric Configuration", fontsize=10, pad=12
    )
...
def fig_train_vs_test():
    ...
    for ax, (s, m) in zip(axes, best_configs):
        ...
        ax.scatter(train_delta, test_delta, c=scatter_c, s=30, edgecolors="white", linewidth=0.3, alpha=0.85, zorder=3)
        ...
        for i, (td, te, prot) in enumerate(zip(train_delta, test_delta, proteins)):
            if abs(td - te) > threshold:
                ax.annotate(prot, (td, te), fontsize=5.5, alpha=0.8, xytext=(3, 3), textcoords="offset points")
        ...
        ax.text(0.05, 0.95, f"r = {r_val:.2f}", transform=ax.transAxes, fontsize=7.5, va="top")
...
def fig_delta_distribution():
    ...
    fig.suptitle("Distribution of Per-Protein Test-Set Improvement (Optimised − Baseline)", fontsize=10, y=1.01)
    ...
    for ax, m_key in zip(axes, METRICS):
        ...
        for i, s in enumerate(SCORING):
            ...
            med = vals.median()
            ax.text(i, vals.max() + 0.005, f"med={med:+.3f}", ha="center", fontsize=6.5, color=PALETTE[s])
...
def fig_lollipop_best():
    ...
    ax.set_title(f"Per-Protein Test ROC-AUC\n{SCORING_LABELS[s]} scoring, optimised for {METRIC_LABELS[m]}", fontsize=9)
...
def fig_summary_table():
    ...
    ax.set_title("Summary: Average Test-Set Performance Across All 9 Configurations", fontsize=10, pad=20, y=0.95)
    plt.tight_layout()
...
def fig_threshold_heatmap():
    ...
    clean_keys = [
        k.replace("_gnina", "\n(GNINA)").replace("_diffdock", "\n(DiffDock)").replace("%", "Pct").replace("_", " ")
        for k in all_keys
    ]
    ...
    ax.set_yticklabels(proteins, fontsize=7)
    ax.set_title(
        "Optimised Threshold Values per Protein\n(Z-scored per parameter; CNNaffinity, ROC-AUC)", fontsize=9, pad=10
    )
