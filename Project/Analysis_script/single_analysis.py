...
def calculate_contact_surface_area(ligand_selection, binding_site_selection):
    ...
    cmd.delete("isolated_ligand")
    cmd.delete("isolated_binding_site")
...
def merge_confidence_contact(
    ...
    occupancy_column = [col for col in df1.columns if re.match(r"%occupancy.*", col)]
    ...
    if atom_column and confidence_column:
        ...
        df1[f"Final_{final_name}"] = (
            ...
            & (df1[occupancy_column[0]] >= occupancy_threshold)
        ).astype(int)
...
def process_ligand_diffdock(ligand_file, args, total_sasa):
    ...
    if args.reference_ligand:
        ...
        binding_site_selection_str = (
            f"byres (protein within {args.distance} of ref_ligand)"
        )
    ...
    if args.metric == "diffdock":
        ...
        with open(output_file_path, "w", newline="") as csvfile:
            ...
            writer.writerow(
                [
                    "Compounds",
                    f"unoccupied_{os.path.basename(args.output_dir)}",
                    f"%occupancy_{os.path.basename(args.output_dir)}",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Calculate the surface contact area and atom percentage occupancy "
            "of ligands in a protein binding site."
        )
    )
    parser.add_argument(
