...
def fetch_and_process_pdb(
    ...
    cofactor_dir = os.path.join(output_dir, "cofactors")
    cofactor_path = os.path.join(cofactor_dir, f"{pdb_id}_cofactors.pdb")
    ...
    if cofactors:
        ...
        if cmd.count_atoms("cofactors") != 0:
            cmd.save(cofactor_path, "cofactors")
        else:
            cofactor_path = None
        cmd.remove("cofactors")
...
def fix_and_minimize_pdb(
    ...
    try:
        ...
        print("Using CUDA platform for OpenMM.")
    except Exception:
        platform = Platform.getPlatformByName("CPU")
        platform_properties = {"Threads": "10"}
...
def main(args):
    ...
    for index, row in df.iterrows():
        try:
            ...
            chains = (
                [str(chain.strip()).upper() for chain in row["chains"].split("+")]
