...
Usage:
    python optuna_combine_all_structure.py --protein ABL1 --dude-labels
    python optuna_combine_all_structure.py --protein EGFR --dude-labels --n-trials 200
    python optuna_combine_all_structure.py --protein ABL1 --dude-labels --metric pr-auc
"""
...
class AllStructureReranker:
    ...
    def __init__(
        ...
        Args:
            ...
            dude_labels: Derive labels using the DUD-E convention: compounds
                whose identifiers start with ``ZINC`` are decoys and all
                remaining identifiers are actives. This is explicit rather
                than automatic so unrelated datasets are never mislabeled.
        """
        ...
        if self.data_dir is not None:
            ...
            split_file = f"{protein}_{self.split}_final.csv"
            legacy_file = f"{protein}_final.csv"
            self.gnina_csv_path = self._resolve_engine_csv(
                Path(base_dir),
                "gnina",
                protein,
                split_file,
            ...
            self.diffdock_csv_path = self._resolve_engine_csv(
                Path(base_dir),
                "diffdock",
                protein,
                split_file,
    ...
    def _resolve_engine_csv(
        ...
        """Resolve nested campaign exports or the repository's flat CSVs."""
        nested = base_dir / engine / protein / "Confidence_score"
        flat = base_dir / f"result_{engine}"
        candidates = [nested / split_file, flat / split_file]
        if self.split == "train":
            candidates.extend([nested / legacy_file, flat / legacy_file])
    ...
    def _resolve_activity_labels(self) -> None:
        """Populate the configured activity column or fail with guidance."""
        for candidate in (
            self.activity_column,
            f"{self.activity_column}_gnina",
            f"{self.activity_column}_diffdock",
        ):
            if candidate in self.data.columns:
                if candidate != self.activity_column:
                    self.data[self.activity_column] = self.data[candidate]
                logger.info("Using '%s' as activity column", candidate)
                return

        if self.dude_labels:
            compounds = self.data["molecule"].astype(str)
            self.data[self.activity_column] = (~compounds.str.upper().str.startswith("ZINC")).astype(int)
            logger.info("Derived DUD-E labels from compound identifiers: " "ZINC*=decoy, all other identifiers=active")
            if compounds.str.upper().eq("NONE").any():
                logger.warning(
                    "The input contains a compound named 'NONE'; verify that " "this target has a valid active record."
                )
            return

        raise KeyError(
            f"Activity column '{self.activity_column}' not found in data. "
            "Provide an embedded label column or pass --dude-labels for the "
            "repository's DUD-E benchmark CSVs."
        )
    ...
    def _reshape_merged_wide_to_long(self, merged_df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        rank_cols = [
            c for c in merged_df.columns if "_rank" in c and not c.startswith("Final_") and not c.startswith("Clashing")
        ]
    ...
    def _identify_metrics(self):  # noqa: C901
        ...
        if self.scoring_metric == "cnn-combined":
            ...
            affinity_metrics = [
                m for m in self.scoring_metrics if "affinity" in m.lower() and "cnn" not in m.lower() and "_gnina" in m
            ]
            if not affinity_metrics:
                # Try diffdock as fallback
                affinity_metrics = [
                    m for m in self.diffdock_metrics if "affinity" in m.lower() and "cnn" not in m.lower()
                ]
                if not affinity_metrics:
    ...
    def _calculate_cnn_combined_score(self, data: pd.DataFrame, return_scaled: bool = False):  # noqa: C901
        ...
        if cnnpose_col is None or cnnaffinity_col is None:
            logger.warning(
                f"Missing columns for CNN combined score: " f"CNNpose={cnnpose_col}, CNNaffinity={cnnaffinity_col}"
            )
    ...
    def _extract_scores(self, predictions: pd.DataFrame) -> np.ndarray:
        ...
        if self.scoring_metric == "cnn-combined":
            if "cnn_combined_score" in predictions.columns:
                ...
                raw = self._calculate_cnn_combined_score(predictions, return_scaled=False).values.astype(float)
        else:
            scoring_col = None
            if self.scoring_metric == "affinity":
                for col in self.scoring_metrics:
        ...
        if len(finite) == 0:
            raise ValueError(f"All score values are NaN/Inf for scoring_metric='{self.scoring_metric}'.")
...
class AllStructureOptunaOptimizer:
    ...
    def __init__(
        self, reranker: AllStructureReranker, n_trials: int = 100, n_jobs: int = 1, optuna_config: Dict = None
    ):
            ...
            if metric in top_k_data.columns and not top_k_data[metric].isna().all():
                ...
                if len(values) > 10:  # Only apply outlier filtering if we have enough data
                    ...
                    if len(values) != original_count:
                        ...
                        logger.warning(
                            f"  Original range: [{top_k_data[metric].min():.6f}, " f"{top_k_data[metric].max():.6f}]"
                        )
                ...
                thresholds[metric] = trial.suggest_float(f"threshold_{metric}", low=low, high=high)
    ...
    def optimize(self, direction: str = "maximize") -> Dict[str, float]:
        ...
        logger.info(
            f"  - DiffDock threshold metrics: " f"{[m for m in self.reranker.threshold_metrics if '_diffdock' in m]}"
        )
    ...
    def evaluate_thresholds(self, thresholds: Dict[str, float] = None) -> Dict[str, float]:
        ...
        logger.info(
            f"  DiffDock threshold metrics: " f"{len([m for m in self.reranker.threshold_metrics if '_diffdock' in m])}"
        )
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Combined GNINA+DiffDock Optuna reranking", formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )

    # Data arguments
    parser.add_argument("--protein", type=str, required=True, help="Protein target (e.g., ABL1, EGFR, AA2AR)")
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "exports (ignored if --data-dir is set)"
        ),
    )
    ...
    parser.add_argument("--skip-compounds", nargs="*", default=[], help="List of compound names to skip/exclude")
    ...
    parser.add_argument("--n-trials", type=int, default=200, help="Number of Optuna trials to run")
    ...
    parser.add_argument("--save-study", action="store_true", help="Save the Optuna study object for later analysis")
...
def main():
    ...
    try:
        # Initialize reranker
        logger.info("Starting optimization for %s", args.protein)
        ...
        if getattr(args, "eval_on_test", False) and args.split == "train":
            ...
            for key in [
                "optimized_roc_auc",
                "baseline_roc_auc",
                f"optimized_{args.metric}",
                f"baseline_{args.metric}",
                f"{args.metric}_improvement",
                "roc_auc_improvement",
            ]:
            ...
            metric_display = {"roc-auc": "ROC-AUC", "pr-auc": "PR-AUC", "logauc": "logAUC"}.get(
        ...
        # Print summary
        print(f"\n{'='*70}")
        print(f"OPTIMIZATION RESULTS FOR {args.protein.upper()}")
        print(f"{'='*70}")
