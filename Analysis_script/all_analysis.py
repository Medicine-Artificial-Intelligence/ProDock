...
"""
        ...
        # Step 3: Run APBS
        with open(apbs_output, "w") as output_file:
            subprocess.run(
                ["apbs", apbs_input],
                stdout=output_file,
        ...
        # Step 4: Extract solvation energy from APBS output
        solv_energy = None

        with open(apbs_output, "r") as file:
            for line in file:
                if "Total electrostatic energy" in line:
                    solv_energy = float(line.split()[-2])
                    break

        if solv_energy is None:
            return None
        ...
        for intermediate_file in [pqr_file, apbs_input, apbs_output]:
            try:
...
def copy_and_rename_files(source_dir, destination_dir, num_conform, n_jobs):
    ...
    confidence_dir = os.path.join(all_dir, "Combined_score")
...
def run_occupancy(
    ...
    for i in range(1, n_conf + 1):
        ...
        if occupancy_threshold:
            cmd.extend(["--occupancy_threshold", str(occupancy_threshold)])
...
def merge_diffdock_matrix(
    ...
    output_file_path = os.path.join(
        output_directory,
        f"all_best_satisfied_occupancy_atom{atom}_occupancy{occupancy}_confidence{confidence}_distance{distance}.csv",
    )
...
def visualization_diffdock(
    ...
    # Add a colorbar
    cbar = plt.colorbar(heatmap, ax=ax, orientation="vertical", shrink=0.8)
    cbar.set_label("occupancy", fontsize=font_size * 1.2, fontweight="bold")
    cbar.ax.tick_params(labelsize=font_size * 0.9)
    cbar.ax.invert_yaxis()
    ax.set_xlabel("Proteins", fontsize=font_size * 1.2, fontweight="bold", labelpad=15)
    ax.set_ylabel("Compounds", fontsize=font_size * 1.2, fontweight="bold", labelpad=15)
    # Set title and layout
    original_title = "occupancy of the screening compounds (y axis) and the protein targets (x axis) heatmap"
    # Dynamically wrap the title into two lines
    wrapped_title = "\n".join(textwrap.wrap(original_title, width=cols * 7))
    ...
    output_path = os.path.join(
        base_directory,
        "Visualization",
        f"all_best_satisfied_occupancy_atom{atom}_occupancy{occupancy}_confidence{confidence}_distance{distance}.tiff",
    )
    plt.savefig(output_path, dpi=300, format="tiff")
    output_path = os.path.join(
        base_directory,
        "Visualization",
        f"all_best_satisfied_occupancy__atom{atom}_occupancy{occupancy}_confidence{confidence}_distance{distance}.png",
    )
    plt.savefig(output_path, dpi=300, format="png")
    return os.path.join(base_directory, "Visualization")
...
def merge_final_gnina(base_directory):
    ...
    matrix_df_affinity["Best_satisfied_affinity"] = np.nan
    matrix_df_affinity["Best_satisfied_rank"] = pd.NA

    matrix_df_cnnpose = merged_final_df[["Compounds"]].copy()
    matrix_df_cnnpose["Best_satisfied_cnnpose"] = np.nan
    matrix_df_cnnpose["Best_satisfied_rank"] = pd.NA

    matrix_df_cnnaffinity = merged_final_df[["Compounds"]].copy()
    matrix_df_cnnaffinity["Best_satisfied_cnnaffinity"] = np.nan
    matrix_df_cnnaffinity["Best_satisfied_rank"] = pd.NA
        ...
        if row["Satisfied_count"] > 0:
            for i in range(len(final_columns)):
                if row[final_columns[i]] != 0:
                    ...
                    matrix_df_affinity.at[index, "Best_satisfied_affinity"] = pd.to_numeric(
                        row[affinity_columns[i]], errors="coerce"
                    )
                    matrix_df_affinity.at[index, "Best_satisfied_rank"] = f"rank{i+1}"

                    matrix_df_cnnpose.at[index, "Best_satisfied_cnnpose"] = pd.to_numeric(
                        row[cnnpose_columns[i]], errors="coerce"
                    )
                    matrix_df_cnnpose.at[index, "Best_satisfied_rank"] = f"rank{i+1}"

                    matrix_df_cnnaffinity.at[index, "Best_satisfied_cnnaffinity"] = pd.to_numeric(
                        row[cnnaffinity_columns[i]], errors="coerce"
                    )
                    matrix_df_cnnaffinity.at[index, "Best_satisfied_rank"] = f"rank{i+1}"
                    break
    with open(
        os.path.join(
            base_directory,
...
def merge_final_diffdock(base_directory):
    ...
    matrix_df["Best_satisfied_occupancy"] = np.nan
    matrix_df["Best_satisfied_rank"] = pd.NA
    for index, row in merged_final_df.iterrows():
        if row["Satisfied_count"] > 0:
            for i in range(len(final_columns)):
                if row[final_columns[i]] != 0:
                    screened_withscore_df.append(
                        [
                            row["Compounds"],
                            f"rank{i+1}",
                            row[unoccupied_columns[i]],
                            row[percent_unoccupied_columns[i]],
                            row[atom_columns[i]],
                        ]
                    )
                    matrix_df.at[index, "Best_satisfied_occupancy"] = pd.to_numeric(
                        row[unoccupied_columns[i]], errors="coerce"
                    )
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=20.0,
        help="Threshold for percentage of occupancy column (recommeded for DiffDock)",
    )
    parser.add_argument(
        "--confidence_threshold",
        type=float,
        ...
        if (
            ...
            if args.protein_dir:
                ...
                run_occupancy(
                    ...
                    occupancy_threshold=str(args.occupancy_threshold),
