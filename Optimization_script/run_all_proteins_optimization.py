#!/usr/bin/env python3
"""
Batch script to run Optuna optimization for all proteins in /all directory structure

This script automatically discovers all available proteins in the /all directory
and runs the combined Gnina+DiffDock optimization for each target.

Usage:
    python run_all_proteins_optimization.py --base-dir all
    python run_all_proteins_optimization.py --base-dir all --n-trials 100 --parallel 4
    python run_all_proteins_optimization.py --base-dir all --metric pr-auc --parallel 2
"""
...
def discover_proteins(base_dir: str) -> List[str]:
    """
    ...
    """
    gnina_dir = Path(base_dir) / "gnina"
    diffdock_dir = Path(base_dir) / "diffdock"

    if not gnina_dir.exists():
        logger.error(f"Gnina directory not found: {gnina_dir}")
        return []

    if not diffdock_dir.exists():
        logger.error(f"DiffDock directory not found: {diffdock_dir}")
        return []

    # Get proteins with gnina data
    gnina_proteins = set()
    for protein_dir in gnina_dir.iterdir():
        if protein_dir.is_dir():
            final_csv = protein_dir / "Confidence_score" / f"{protein_dir.name}_final.csv"
            if final_csv.exists():
                gnina_proteins.add(protein_dir.name)
                logger.debug(f"Found gnina data for: {protein_dir.name}")

    # Get proteins with diffdock data
    diffdock_proteins = set()
    for protein_dir in diffdock_dir.iterdir():
        if protein_dir.is_dir():
            final_csv = protein_dir / "Confidence_score" / f"{protein_dir.name}_final.csv"
            if final_csv.exists():
                diffdock_proteins.add(protein_dir.name)
                logger.debug(f"Found diffdock data for: {protein_dir.name}")
...
def run_single_optimization(protein: str, args) -> dict:  # noqa: C901
    ...
    cmd = [
        sys.executable, "optuna_combine_all_structure.py",
        "--protein", protein,
        "--base-dir", args.base_dir,
        "--scoring-metric", args.scoring_metric,
        "--metric", args.metric,
...
def save_batch_results(results: List[dict], output_file: str):
    ...
    if with_test:
        ...
        summary['avg_test_baseline_roc_auc'] = (
            sum(r.get('test_baseline_roc_auc', 0) for r in with_test) / len(with_test)
        )
        summary['avg_test_roc_auc_improvement'] = (
            sum(r.get('test_roc_auc_improvement', 0) for r in with_test) / len(with_test)
        )
        ...
        if test_opt_key in with_test[0]:
            ...
            summary[f'avg_test_baseline_{metric}'] = (
                sum(r.get(f'test_baseline_{metric}', 0) for r in with_test) / len(with_test)
            )
            summary[f'avg_test_{metric}_improvement'] = (
                sum(r.get(f'test_{metric}_improvement', 0) for r in with_test) / len(with_test)
            )
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Batch Optuna optimization for all proteins in /all structure",
    ...
    parser.add_argument(
        ...
        type=str,
        default="all",
        help="Base directory containing gnina/ and diffdock/ folders (ignored if --split-merged-dir is set)"
    )
    ...
    parser.add_argument(
        "--proteins",
        nargs="*",
        help="Specific proteins to process (if not specified, process all found)"
    )
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=[],
        help="Proteins to exclude from processing"
    )
    ...
    #     type=str,
    #     default="active_",
    #     help="Prefix for active compounds"
    # )
    ...
    #     type=str,
    #     default="decoy_",
    #     help="Prefix for decoy compounds"
    # )
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Number of Optuna trials per protein"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=3600,
        help="Timeout per protein in seconds (default: 1 hour)"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of proteins to process in parallel (default: 1, sequential)"
    )
    ...
    parser.add_argument(
        "--skip-temp-compounds",
        action="store_true",
        help="Temporary option to skip compounds L000-L014"
    )
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=[],
        help="Custom list of compound names to skip/exclude"
    )
    ...
    parser.add_argument(
        "--save-study",
        action="store_true",
        help="Save Optuna study objects for each protein"
    )
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Show what would be processed without running optimization"
    )

    # Activity CSV arguments
    # parser.add_argument(
    #     "--activity-csv",
    #     type=str,
    #     help="Path to activity CSV file"
    # )

    parser.add_argument(
        "--activity-column",
        type=str,
        help="Column name in activity CSV file"
    )
...
def main():
    ...
    if getattr(args, 'split_merged_dir', None):
        ...
        all_proteins = discover_proteins(args.base_dir)
        if not all_proteins:
    ...
    if args.parallel <= 1:
        ...
        with ProcessPoolExecutor(max_workers=args.parallel) as executor:
            # Submit all jobs
            future_to_protein = {
                executor.submit(run_single_optimization, protein, args): protein
                for protein in proteins_to_process
            }
