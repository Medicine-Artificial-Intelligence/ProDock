...
class MOAD(Dataset):
    def __init__(
        ...
        self.prot_cache_path = os.path.join(
            cache_path,
            f"MOAD12_limit{self.limit_complexes}_INDEX{self.split}"
        ...
        print(
            "loading ligands from memory: ",
            os.path.join(self.lig_cache_path, "ligands.pkl"),
        )
        ...
        if require_ligand:
            with open(
                os.path.join(self.lig_cache_path, "rdkit_ligands.pkl"), "rb"
            ) as f:
                self.rdkit_ligands = pickle.load(f)
                self.rdkit_ligands = {
                    lig.name: mol for mol, lig in zip(self.rdkit_ligands, self.ligands)
                }
        ...
        if not self.single_cluster_name is None:
            self.ligands = [
                ...
                if lig.name in self.cluster_to_ligands[self.single_cluster_name]
            ]
        print(
            "Kept",
            len(self.ligands),
            f"ligands in {self.single_cluster_name} out of",
            len_before,
        )
        ...
        self.ligands = {
            lig.name: lig
            for lig in self.ligands
            if min_ligand_size == 0 or lig["ligand"].x.shape[0] >= min_ligand_size
        }
        print(
            "removed",
            len_before - len(self.ligands),
            "ligands below minimum size out of",
            len_before,
        )

        receptors_names = set([lig.name[:6] for lig in self.ligands.values()])
        self.collect_receptors(
            receptors_names, max_receptor_size, remove_promiscuous_targets
        )

        # filter ligands for which the receptor failed
        tot_before = len(self.ligands)
        self.ligands = {
            k: v for k, v in self.ligands.items() if k[:6] in self.receptors
        }
        print(
            "removed",
            tot_before - len(self.ligands),
            "ligands with no receptor out of",
            tot_before,
        )

        if remove_pdbbind:
            complexes_pdbbind = read_strings_from_txt(
                "data/splits/timesplit_no_lig_overlap_train"
            ) + read_strings_from_txt("data/splits/timesplit_no_lig_overlap_val")
            with open(
                "data/BindingMOAD_2020_ab_processed_biounit/ecod_t_group_binding_site_assignment_dict_major_domain.pkl",
                "rb",
            ) as f:
                pdbbind_to_cluster = pickle.load(f)
            clusters_pdbbind = set([pdbbind_to_cluster[c] for c in complexes_pdbbind])
            self.split_clusters = [
                c for c in self.split_clusters if c not in clusters_pdbbind
            ]
            self.cluster_to_ligands = {
                k: v
                for k, v in self.cluster_to_ligands.items()
                if k not in clusters_pdbbind
            }
            ligand_accepted = []
            ...
            tot_before = len(self.ligands)
            self.ligands = {
                k: v for k, v in self.ligands.items() if k in ligand_accepted
            }
            print(
                "removed",
                tot_before - len(self.ligands),
                "ligands in overlap with PDBBind out of",
                tot_before,
            )
        ...
        if enforce_timesplit:
            ...
            self.ligands = {
                ...
                if k[:4].upper() not in pdbids_from2019
            }
            print(
                "removed",
                len_before - len(self.ligands),
                "ligands from 2019 out of",
                len_before,
            )
        ...
        if unroll_clusters:
            ...
            self.cluster_to_ligands = {
                k: [k2 for k2 in self.ligands.keys() if k2[:6] == k] for k in rec_keys
            }
            self.split_clusters = list(rec_keys)
        else:
            for c in self.cluster_to_ligands.keys():
                self.cluster_to_ligands[c] = [
                    v for v in self.cluster_to_ligands[c] if v in self.ligands
                ]
            self.split_clusters = [
        ...
        list_names = [
            ...
            for name in self.cluster_to_ligands[cluster]
        ]
        with open(
            os.path.join(self.prot_cache_path, f"moad_{self.split}_names.txt"), "w"
        ) as f:
            f.write("\n".join(list_names))
    ...
    def get_by_name(self, ligand_name, cluster):
        ...
        if False and self.keep_original and hasattr(ligand_graph["ligand"], "orig_pos"):
            lig_path = os.path.join(
                self.moad_dir, "pdb_superligand", ligand_name + ".pdb"
            )
            ...
            for ligand_comp in self.cluster_to_ligands[cluster]:
                ...
                lig_path_comp = os.path.join(
                    self.moad_dir, "pdb_superligand", ligand_comp + ".pdb"
                )
                if not os.path.exists(lig_path_comp):
                    continue

                lig_comp = Chem.MolFromPDBFile(lig_path_comp)
                formula_comp = np.asarray(
                    [atom.GetSymbol() for atom in lig_comp.GetAtoms()]
                )

                if (
                    formula.shape == formula_comp.shape
                    and np.all(formula == formula_comp)
                    and hasattr(self.ligands[ligand_comp], "orig_pos")
                ):
                    print(
                        f"Found complex {ligand_comp} to have the same complex/ligand pair, adding it into orig_pos"
                    )
                    # add the orig_pos of the binding position
                    if not isinstance(ligand_graph["ligand"].orig_pos, list):
                        ligand_graph["ligand"].orig_pos = [
                            ligand_graph["ligand"].orig_pos
                        ]
                    ligand_graph["ligand"].orig_pos.append(
        ...
        if self.chain_cutoff:
            distances = torch.norm(
                (
                    torch.from_numpy(complex_graph["ligand"].orig_pos[0])
                    - complex_graph.original_center
                ).unsqueeze(1)
                - complex_graph["receptor"].pos.unsqueeze(0),
                dim=2,
            )
            ...
            chains_within_cutoff = torch.zeros(
                torch.max(complex_graph["receptor"].chain_ids) + 1
            )
            chains_within_cutoff.index_add_(
                0, complex_graph["receptor"].chain_ids, within_cutoff.float()
            )
            chains_within_cutoff_bool = chains_within_cutoff > 0
            residues_to_keep = chains_within_cutoff_bool[
                complex_graph["receptor"].chain_ids
            ]

            if self.all_atoms:
                atom_to_res_mapping = complex_graph[
                ...
                atom_to_res_new_mapping = rec_remapper[atom_to_res_mapping][
                    atoms_to_keep
                ]
                atom_res_edge_index = torch.stack(
                    [
                        torch.arange(len(atom_to_res_new_mapping)),
            ...
            complex_graph["receptor"].pos = complex_graph["receptor"].pos[
                residues_to_keep
            ]
            complex_graph["receptor"].x = complex_graph["receptor"].x[residues_to_keep]
            complex_graph["receptor"].side_chain_vecs = complex_graph[
                "receptor"
            ].side_chain_vecs[residues_to_keep]
            complex_graph["receptor", "rec_contact", "receptor"].edge_index = subgraph(
                residues_to_keep,
                complex_graph["receptor", "rec_contact", "receptor"].edge_index,
                relabel_nodes=True,
            )[0]

            extra_center = torch.mean(
    ...
    def preprocessing_receptors(self):
        print(
            f"Processing receptors from [{self.split}] and saving it to [{self.prot_cache_path}]"
        )

        complex_names_all = sorted(
            [l for c in self.split_clusters for l in self.cluster_to_ligands[c]]
        )
        ...
        for i in list_indices:
            ...
            with tqdm(
                ...
                for t in map_fn(
                    self.get_receptor,
                    zip(
                        receptor_names, [sequences_to_embeddings] * len(receptor_names)
                    ),
                ):
                    if t is not None:
            ...
            with open(
                os.path.join(self.prot_cache_path, f"receptors{i}.pkl"), "wb"
            ) as f:
    ...
    def check_all_receptors(self):
        ...
        for i in range(len(receptor_names_all) // 1000 + 1):
            if not os.path.exists(
                os.path.join(self.prot_cache_path, f"receptors{i}.pkl")
            ):
                return False
        return True
    ...
    def collect_receptors(
        ...
        total_recovered = 0
        print(
            f"Loading {len(receptor_names_all)} receptors to keep {len(receptors_to_keep)}."
        )
        for i in range(len(receptor_names_all) // 1000 + 1):
            print(
                f'prot path: {os.path.join(self.prot_cache_path, f"receptors{i}.pkl")}'
            )
            with open(
                os.path.join(self.prot_cache_path, f"receptors{i}.pkl"), "rb"
            ) as f:
                l = pickle.load(f)
        ...
        if max_receptor_size is not None:
            receptor_graphs_all = [
                ...
                if rec["receptor"].pos.shape[0] <= max_receptor_size
            ]
            print(
        ...
        if remove_promiscuous_targets is not None:
            ...
            receptor_graphs_all = [
                rec
                for rec in receptor_graphs_all
                if rec["receptor_name"] not in promiscuous_targets
            ]
            print(
    ...
    def preprocessing_ligands(self):
        print(
            f"Processing complexes from [{self.split}] and saving it to [{self.lig_cache_path}]"
        )
        ...
        for i in list_indices:
            ...
            with open(
                os.path.join(self.lig_cache_path, f"rdkit_ligands{i}.pkl"), "wb"
            ) as f:
        ...
        for i in range(len(complex_names_all) // 1000 + 1):
            with open(
                os.path.join(self.lig_cache_path, f"rdkit_ligands{i}.pkl"), "rb"
            ) as f:
                l = pickle.load(f)
    ...
    def get_ligand(self, name):
        ...
        try:
            if self.matching:
                ...
                if "." in smile:
                    print(
                        f"Ligand {name} has multiple fragments and we are doing matching. Not including {name} in preprocessed data."
                    )
                    return None
        ...
        if self.split != "train":
            ...
            for i in range(100):
                new_file = os.path.join(
                    self.moad_dir,
                    "pdb_ligand",
                    f"{nsplit[0]}_{nsplit[1]}_{nsplit[2]}_{i}.pdb",
                )
...
def print_statistics(dataset):
    ...
    for i in range(len(dataset)):
        ...
        statistics[4].append(
            int(complex_graph.random_coords) if "random_coords" in complex_graph else -1
        )
        if (
            "random_coords" in complex_graph
            and complex_graph.random_coords
            and "rmsd_matching" in complex_graph
        ):
