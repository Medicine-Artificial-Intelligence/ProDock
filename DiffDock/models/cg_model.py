...
class CGModel(torch.nn.Module):
    def __init__(
        ...
        if sidechain_pred:
            self.sidechain_predictor = Linear(
                irreps_in=irrep_seq[
                    min(num_prot_emb_layers + num_conv_layers, len(irrep_seq) - 1)
                ],
        ...
        if self.confidence_mode:
            ...
            self.confidence_predictor = nn.Sequential(
                ...
                nn.Dropout(confidence_dropout),
                nn.Linear(
                    ns, num_confidence_outputs + (1 if self.affinity_prediction else 0)
                ),
            )
            ...
            for l in range(len(self.rec_emb_layers)):
                edge_attr_ = torch.cat(
                    [
                        rec_edge_attr,
                        rec_node_attr[
                            data["receptor", "receptor"].edge_index[0], : self.ns
                        ],
                        rec_node_attr[
                            data["receptor", "receptor"].edge_index[1], : self.ns
                        ],
                    ],
                    -1,
                )
    ...
    def forward(self, data):
        ...
        lr_edge_index, lr_edge_attr, lr_edge_sh, rev_lr_edge_sh, lr_edge_weight = (
        ...
        edge_attr = torch.cat(
            [lig_edge_attr, lr_edge_attr, rec_edge_attr, lr_edge_attr], dim=0
        )
        edge_sh = torch.cat(
            [lig_edge_sh, lr_edge_sh, rec_edge_sh, rev_lr_edge_sh], dim=0
        )
        edge_weight = (
            torch.cat(
                [lig_edge_weight, lr_edge_weight, rec_edge_weight, lr_edge_weight],
                dim=0,
            )
        ...
        if self.sidechain_pred:
            ...
            sidechain_pred = (
                sidechain_pred[:, :10] + sidechain_pred[:, 10:]
            )  # sum even and odd components
    ...
    def build_rec_conv_graph(self, data):
        ...
        assert (
            not self.separate_noise_schedule or self.asyncronous_noise_schedule
        ), "removed support in this function"
    ...
    def build_misc_atom_conv_graph(self, data):
        ...
        if self.separate_noise_schedule:
            data["misc_atom"].node_sigma_emb = torch.cat(
                [
                    self.timestep_emb_func(data["misc_atom"].node_t[noise_type])
                    for noise_type in ["tr", "rot", "tor"]
                ],
                dim=1,
            )
        elif self.asyncronous_noise_schedule:
            data["misc_atom"].node_sigma_emb = self.timestep_emb_func(
                data["misc_atom"].node_t["t"]
            )
        else:
            data["misc_atom"].node_sigma_emb = self.timestep_emb_func(
                data["misc_atom"].node_t["tr"]
            )  # tr rot and tor noise is all the same
        node_attr = torch.cat(
            [data["misc_atom"].x, data["misc_atom"].node_sigma_emb], 1
        )
    ...
    def build_misc_cross_conv_graph(self, data, lr_cross_distance_cutoff):
        ...
        la_edge_vec = (
            data["misc_atom"].pos[la_edge_index[1].long()]
        ...
        ar_edge_vec = (
            ...
            - data["misc_atom"].pos[ar_edge_index[0].long()]
        )
