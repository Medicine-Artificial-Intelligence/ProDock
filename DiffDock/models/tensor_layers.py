...
def tp_scatter_multigroup(
    ...
    assert isinstance(
        edge_attr_groups, list
    ), "This function is only for a list of edge groups"
    ...
    for ii in range(num_edge_groups):
        ...
        summand = tp(
            node_attr[cur_edge_dst, :], edge_sh[cur_edge_range, :], cur_out_irreps
        )
        # We take a simple sum, and then add up the count of edges which contribute,
        # so that we can take the mean later.
        final_out += scatter(
            summand, cur_edge_src, dim=0, dim_size=out_nodes, reduce="sum"
        )
...
class TensorProductConvLayer(torch.nn.Module):
    ...
    def forward(
        ...
        if edge_index.shape[1] == 0:
            out = torch.zeros(
                (node_attr.shape[0], self.out_size),
                dtype=_dtype,
