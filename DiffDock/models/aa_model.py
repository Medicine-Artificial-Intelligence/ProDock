...
class AAModel(torch.nn.Module):
    def __init__(
        ...
        assert (
            not sidechain_pred
        ), "sidechain prediction not implemented/makes sense for all atom model"
        assert (
            not depthwise_convolution
        ), "depthwise convolution not implemented for all atom model"
        if parallel > 1:
        ...
        for i in range(num_prot_emb_layers, num_prot_emb_layers + num_conv_layers):
            ...
            layer = TensorProductConvLayer(
                ...
                edge_groups=(
                    ...
                    else (3 if i == num_prot_emb_layers + num_conv_layers - 1 else 9)
                ),
            )
        ...
        if self.confidence_mode:
            ...
            if not no_torsion:
                ...
                self.tor_bond_conv = TensorProductConvLayer(
                    ...
                    out_irreps=(
                        f"{ns}x0o + {ns}x0e" if not self.odd_parity else f"{ns}x0o"
                    ),
                    n_edge_features=3 * ns,
                    residual=False,
                    dropout=dropout,
    ...
    def embedding(self, data):
        if not hasattr(data["receptor"], "rec_node_attr"):
                ...
                rec_lm_emb = torch.cat(
                    [
                        ...
                        for i, t in enumerate(
                            out["representations"][self.lm.num_layers]
                        )
                    ],
                    dim=0,
                )
            ...
            rec_node_attr, rec_edge_attr, rec_edge_sh, rec_edge_weight = (
                self.build_rec_conv_graph(data)
            )
            rec_node_attr = self.rec_node_embedding(rec_node_attr)
            rec_edge_attr = self.rec_edge_embedding(rec_edge_attr)

            atom_node_attr, atom_edge_attr, atom_edge_sh, atom_edge_weight = (
            ...
            ar_edge_attr, ar_edge_sh, ar_edge_weight = self.build_cross_rec_conv_graph(
                data
            )
            ...
            edge_sh = torch.cat(
                [rec_edge_sh, ar_edge_sh, atom_edge_sh, ar_edge_sh], dim=0
            )
            edge_weight = (
                torch.cat(
        ...
        # receptor embedding
        rec_sigma_emb = self.rec_sigma_embedding(
            self.timestep_emb_func(data.complex_t["tr"])
        )
        rec_node_attr = data["receptor"].rec_node_attr + 0
        rec_node_attr[:, : self.ns] = (
            rec_node_attr[:, : self.ns] + rec_sigma_emb[data["receptor"].batch]
        )
        rec_edge_attr = (
            data["receptor", "receptor"].rec_edge_attr
            + rec_sigma_emb[
                data["receptor"].batch[data["receptor", "receptor"].edge_index[0]]
            ]
        )

        # atom embedding
        atom_node_attr = data["atom"].atom_node_attr + 0
        atom_node_attr[:, : self.ns] = (
            atom_node_attr[:, : self.ns] + rec_sigma_emb[data["atom"].batch]
        )
        atom_edge_attr = (
            data["atom", "atom"].atom_edge_attr
            + rec_sigma_emb[data["atom"].batch[data["atom", "atom"].edge_index[0]]]
        )

        # atom-receptor embedding
        ar_edge_attr = (
            data["atom", "receptor"].edge_attr
            + rec_sigma_emb[data["atom"].batch[data["atom", "receptor"].edge_index[0]]]
        )
    ...
    def forward(self, data):
        if self.crop_beyond is not None:
            ...
            residues_to_keep = torch.any(
                torch.sum(
                    (ligand_pos.unsqueeze(0) - receptor_pos.unsqueeze(1)) ** 2, -1
                )
                < self.crop_beyond**2,
                dim=1,
            )
            ...
            data["receptor"].side_chain_vecs = data["receptor"].side_chain_vecs[
                residues_to_keep
            ]
            data["receptor", "rec_contact", "receptor"].edge_index = subgraph(
                residues_to_keep,
                data["receptor", "rec_contact", "receptor"].edge_index,
                relabel_nodes=True,
            )[0]
        ...
        (
            ...
            la_edge_sh,
            la_edge_weight,
        ) = self.build_cross_lig_conv_graph(data, cross_cutoff)
        ...
        for l in range(len(self.conv_layers)):
            if l < len(self.conv_layers) - 1:
                ...
                edge_attr_ = torch.cat(
                    [
                        edge_attr[:s3],
                        node_attr[edge_index[0, :s3], : self.ns],
                        node_attr[edge_index[1, :s3], : self.ns],
                    ],
                    -1,
                )
                if self.differentiate_convolutions:
        ...
        if self.confidence_mode:
            scalar_lig_attr = (
                torch.cat(
                    [
                        ...
                        lig_node_attr[
                            :, -(self.nv if self.reduce_pseudoscalars else self.ns) :
                        ],
                    ],
                    dim=1,
                )
            ...
            if self.atom_confidence:
                ...
                atom_confidence = torch.zeros(
                    (len(lig_node_attr),), device=lig_node_attr.device
                )
            ...
            if self.parallel > 1:
                ...
                affinity = torch.cat(
                    [AGGREGATORS[agg](affinity) for agg in self.parallel_aggregators],
                    dim=-1,
                )
        ...
        tor_bonds, tor_edge_index, tor_edge_attr, tor_edge_sh, tor_edge_weight = (
    ...
    def build_lig_conv_graph(self, data):
        ...
        if self.parallel == 1:
            ...
            batches = torch.zeros(
                data.num_graphs, device=data["ligand"].x.device
            ).long()
            batches = batches.index_add(
                0,
                data["ligand"].batch,
                torch.ones(
                    len(data["ligand"].batch), device=data["ligand"].x.device
                ).long(),
            )
            outer_batches = data.num_graphs
            b = [
                torch.ones(
                    batches[i].item() // self.parallel, device=data["ligand"].x.device
                ).long()
                * (self.parallel * i + j)
            ...
            radius_edges = radius_graph(
                data["ligand"].pos, self.lig_max_radius, data["ligand"].batch_parallel
            )
        edge_index = torch.cat(
