...
def train(
    ...
    freeze_params = 0
    scheduler_mode = (
        args.inference_earlystop_goal if args.val_inference_freq is not None else "min"
    )
    ...
    for epoch in range(args.n_epochs):
        ...
        if (
            args.scheduler == "layer_linear_warmup"
            and (epoch + 1) % args.warmup_dur == 0
        ):
            step = (epoch + 1) // args.warmup_dur
            if step < args.num_conv_layers + 2:
                print("New unfreezing step")
                optimizer, scheduler = get_optimizer_and_scheduler(
                    args, model, step=step, scheduler_mode=scheduler_mode
                )
            ...
            optimizer, scheduler = get_optimizer_and_scheduler(
                args, model, step=1, scheduler_mode=scheduler_mode, optimizer=optimizer
            )
        ...
        if (
            ...
            inf_dataset = [
                ...
                for i in range(
                    min(args.num_inference_complexes, val_loader.dataset.__len__())
                )
            ]
            inf_metrics = inference_epoch_fix(
                model, inf_dataset, device, t_to_sigma, args
            )
            print(
        ...
        if (
            args.double_val
            and args.val_inference_freq != None
            and (epoch + 1) % args.val_inference_freq == 0
        ):
            inf_dataset = [
                val_dataset2.get(i)
                for i in range(
                    min(args.num_inference_complexes, val_dataset2.__len__())
                )
            ]
            inf_metrics2 = inference_epoch_fix(
                model, inf_dataset, device, t_to_sigma, args
            )
            print(
            ...
            logs.update(
                {"valinf2_" + k: v for k, v in inf_metrics2.items()}, step=epoch + 1
            )
            logs.update(
                {
                    "valinfcomb_" + k: (v + inf_metrics[k]) / 2
                    for k, v in inf_metrics2.items()
                },
                step=epoch + 1,
            )
        ...
        if (
            ...
            and (epoch + 1) % args.train_inference_freq == 0
        ):
            inf_dataset = [
                train_loader.dataset.get(i)
                for i in range(
                    min(
                        min(args.num_inference_complexes, 300),
            ...
            logs.update(
                {"traininf_" + k: v for k, v in inf_metrics.items()}, step=epoch + 1
            )
        ...
        if (
            args.inference_secondary_metric is not None
            and args.inference_secondary_metric in logs.keys()
            and (
                args.inference_earlystop_goal == "min"
                and logs[args.inference_secondary_metric] <= best_val_secondary_value
                or args.inference_earlystop_goal == "max"
                and logs[args.inference_secondary_metric] >= best_val_secondary_value
            )
        ):
        ...
        if scheduler:
            if epoch < freeze_params or (
                args.scheduler == "linear_warmup" and epoch < args.warmup_dur
            ):
                scheduler.step()
...
def main_function():
    ...
    if args.restart_dir:
        try:
            dict = torch.load(
                f"{args.restart_dir}/{args.restart_ckpt}.pt",
        ...
        dict = torch.load(
            f"{args.pretrain_dir}/{args.pretrain_ckpt}.pt",
            map_location=torch.device("cpu"),
        )
        model.module.load_state_dict(dict, strict=True)
