...
def get_optimizer_and_scheduler(
    args, model, scheduler_mode="min", step=0, optimizer=None
):
    if args.scheduler == "layer_linear_warmup":
        if step == 0:
            ...
            for l in [
                model.center_edge_embedding,
                model.final_conv,
                model.tr_final_layer,
                model.rot_final_layer,
                model.final_edge_embedding,
                model.final_tp_tor,
                model.tor_bond_conv,
                model.tor_final_layer,
            ]:
                unfreeze_layer(l)
            ...
            for l in [
                model.lig_node_embedding,
                model.lig_edge_embedding,
                model.rec_node_embedding,
                model.rec_edge_embedding,
                model.rec_sigma_embedding,
                model.cross_edge_embedding,
                model.rec_emb_layers,
                model.lig_emb_layers,
            ]:
                unfreeze_layer(l)
...
def get_model(
    args, device, t_to_sigma, no_parallel=False, confidence_mode=False, old=False
):

    timestep_emb_func = get_timestep_embedding(
        embedding_type=(
            args.embedding_type if "embedding_type" in args else "sinusoidal"
        ),
    ...
    if old:
        ...
        model = model_class(
            ...
            affinity_prediction=(
                args.affinity_prediction if "affinity_prediction" in args else False
            ),
            ...
            parallel_aggregators=(
                args.parallel_aggregators if "parallel_aggregators" in args else ""
            ),
            fixed_center_conv=(
                not args.not_fixed_center_conv
                if "not_fixed_center_conv" in args
                else False
            ),
            no_aminoacid_identities=(
                args.no_aminoacid_identities
                if "no_aminoacid_identities" in args
                else False
            ),
            include_miscellaneous_atoms=(
                args.include_miscellaneous_atoms
                if hasattr(args, "include_miscellaneous_atoms")
                else False
            ),
            use_old_atom_encoder=(
                args.use_old_atom_encoder
                if hasattr(args, "use_old_atom_encoder")
                else True
            ),
        )
        ...
        if (
            (
                "moad_esm_embeddings_path" in args
                and args.moad_esm_embeddings_path is not None
            )
            or (
                "pdbbind_esm_embeddings_path" in args
                and args.pdbbind_esm_embeddings_path is not None
            )
            or (
                "pdbsidechain_esm_embeddings_path" in args
                and args.pdbsidechain_esm_embeddings_path is not None
            )
            or ("esm_embeddings_path" in args and args.esm_embeddings_path is not None)
        ):
            lm_embedding_type = "precomputed"
        if "esm_embeddings_model" in args and args.esm_embeddings_model is not None:
        ...
        model = model_class(
            ...
            atom_num_confidence_outputs=(
                ...
                if "atom_rmsd_classification_cutoff" in args
                and isinstance(args.atom_rmsd_classification_cutoff, list)
                else 1
            ),
            parallel_aggregators=(
                args.parallel_aggregators if "parallel_aggregators" in args else ""
            ...
            differentiate_convolutions=(
                not args.no_differentiate_convolutions
                if "no_differentiate_convolutions" in args
                else True
            ),
            tp_weights_layers=(
                args.tp_weights_layers if "tp_weights_layers" in args else 2
            ),
            num_prot_emb_layers=(
                args.num_prot_emb_layers if "num_prot_emb_layers" in args else 0
            ),
            reduce_pseudoscalars=(
                args.reduce_pseudoscalars if "reduce_pseudoscalars" in args else False
            ),
            embed_also_ligand=(
                args.embed_also_ligand if "embed_also_ligand" in args else False
            ),
            atom_confidence=(
                args.atom_confidence_loss_weight > 0.0
                if "atom_confidence_loss_weight" in args
                else False
            ),
            sidechain_pred=(
                hasattr(args, "sidechain_loss_weight")
                and args.sidechain_loss_weight > 0
            )
            or (
                hasattr(args, "backbone_loss_weight") and args.backbone_loss_weight > 0
            ),
            depthwise_convolution=(
                args.depthwise_convolution
                if hasattr(args, "depthwise_convolution")
                else False
            ),
        )
    ...
    if (
        device.type == "cuda"
        and not no_parallel
        and ("dataset" not in args or not args.dataset == "torsional")
    ):
...
def crop_beyond(complex_graph, cutoff, all_atoms):
    ...
    residues_to_keep = torch.any(
        torch.sum((ligand_pos.unsqueeze(0) - receptor_pos.unsqueeze(1)) ** 2, -1)
        < cutoff**2,
        dim=1,
    )
    ...
    if all_atoms:
        ...
        atom_res_edge_index = torch.stack(
            [
                torch.arange(
                    len(atom_to_res_new_mapping), device=atom_to_res_new_mapping.device
                ),
