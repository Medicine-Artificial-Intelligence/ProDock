...
identifiers are decoys and all other identifiers are actives. The archived
`MT1` pair contains `NONE` as its only non-ZINC identifier; it is retained for
archive parity but should be excluded from aggregate results until a valid
active record is recovered.
