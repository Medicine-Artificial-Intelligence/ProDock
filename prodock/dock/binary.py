...
class BinaryDock:
    ...
    def run(self) -> "BinaryDock":
        ...
        # Write driver-captured stdout/stderr to a separate sidecar driver log file.
        # This avoids appending into or clobbering the binary's own log output file.
        try:
            driver_log = self.log_path.with_name(self.log_path.stem + ".driver.log")
            driver_log.parent.mkdir(parents=True, exist_ok=True)
            with open(driver_log, "w", encoding="utf-8") as fh:
                fh.write("--- DRIVER LOG ---\n")
                fh.write(f"called: {called}\n\n")
                fh.write("--- DRIVER STDOUT ---\n")
                fh.write(proc.stdout or "")
                fh.write("\n--- DRIVER STDERR ---\n")
                fh.write(proc.stderr or "")
        except Exception:
            # Swallow IO errors for robustness (we don't want driver log writing to
            # prevent returning the process result), but keep the original logger informed.
            try:
                self._logger.exception("Failed to write driver log")
            except Exception:
                pass
