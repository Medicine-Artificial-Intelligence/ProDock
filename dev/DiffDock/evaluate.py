...
def get_dataset(args, model_args, confidence=False):
    if args.dataset != "moad":
        dataset = PDBBind(
            ...
            atom_max_neighbors=(
                model_args.atom_max_neighbors if "all_atoms" in model_args else None
            ),
            ...
            num_conformers=(
                ...
                if args.resample_rdkit and not confidence
                else 1
            ),
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--old_score_model", action="store_true", default=False, help=""
    )
    parser.add_argument(
        "--old_confidence_model", action="store_true", default=True, help=""
    )
    parser.add_argument(
        "--matching_popsize",
        type=int,
    ...
    parser.add_argument(
        "--moad_esm_embeddings_sequences_path", type=str, default=None, help=""
    )
    parser.add_argument(
        "--chain_cutoff",
        type=float,
        default=None,
        help="Cutoff of the chains from the ligand",
    )  # TODO remove
    parser.add_argument(
        "--save_complexes",
        action="store_true",
        default=False,
        help="Save generated complex graphs",
    )
    ...
    parser.add_argument(
        "--data_dir",
        type=str,
        default="../../ligbind/data/BindingMOAD_2020_ab_processed_biounit/",
        help="Folder containing original structures",
    )
    parser.add_argument(
        "--split_path",
        type=str,
        default="data/BindingMOAD_2020_ab_processed/splits/val.txt",
    ...
    parser.add_argument(
        "--wandb", action="store_true", default=False, help=""
    )  # TODO remove
    parser.add_argument(
        "--inference_steps", type=int, default=40, help="Number of denoising steps"
    )
    parser.add_argument(
        "--limit_complexes",
        type=int,
        default=0,
        help="Limit to the number of complexes",
    )
    parser.add_argument(
        "--num_workers",
        type=int,
        default=1,
        help="Number of workers for dataset creation",
    )
    parser.add_argument(
        "--tqdm",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=str,
        default="expbeta",
        help="Schedule type, no other options",
    )
    parser.add_argument(
        "--inf_sched_alpha",
        type=float,
    ...
    parser.add_argument(
        "--pocket_knowledge", action="store_true", default=False, help=""
    )
    parser.add_argument(
        "--no_random_pocket", action="store_true", default=False, help=""
    )
    ...
    parser.add_argument(
        "--force_fixed_center_conv", action="store_true", default=False, help=""
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--remove_promiscuous_targets", type=float, default=None, help=""
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--gnina_log_file", type=str, default="gnina_log.txt", help=""
    )  # To redirect gnina subprocesses stdouts from the terminal window
    parser.add_argument(
        "--gnina_full_dock", action="store_true", default=False, help=""
    )
    parser.add_argument(
        "--save_gnina_metrics", action="store_true", default=False, help=""
    )
    ...
    with open(f"{args.model_dir}/model_parameters.yml") as f:
        ...
        if not hasattr(
            score_model_args, "separate_noise_schedule"
        ):  # exists for compatibility with old runs that did not have the attribute
    ...
    if args.confidence_model_dir is not None:
        if not (
            ...
            confidence_test_dataset = get_dataset(
                args, confidence_args, confidence=True
            )
    ...
    if not args.no_model:
        ...
        if args.confidence_model_dir is not None:
            if confidence_args.transfer_weights:
                ...
                if not hasattr(
                    confidence_model_args, "separate_noise_schedule"
                ):  # exists for compatibility with old runs that did not have the
                    # attribute
                ...
                if not hasattr(confidence_model_args, "confidence_dropout"):
                    confidence_model_args.confidence_dropout = (
                        confidence_model_args.dropout
                    )
    ...
    if args.pocket_knowledge and args.different_schedules:
        t_max = (np.log(args.pocket_tr_max) - np.log(score_model_args.tr_sigma_min)) / (
            np.log(score_model_args.tr_sigma_max)
            - np.log(score_model_args.tr_sigma_min)
        )
    else:
        t_max = 1
            ...
            try:
                ...
                randomize_position(
                    ...
                    (
                        ...
                        if not args.pocket_knowledge
                        else args.pocket_tr_max
                    ),
                ...
                if args.save_visualisation:
                    ...
                    for idx, graph in enumerate(data_list):
                        ...
                        pdb.add(
                            (
                                (
                                    ...
                                    if not args.resample_rdkit
                                    else orig_complex_graph["ligand"].pos[idx]
                                )
                        ...
                        pdb.add(
                            (graph["ligand"].pos + graph.original_center)
                            .detach()
                            .cpu(),
                            part=1,
                            order=1,
                ...
                filterHs = (
                    torch.not_equal(data_list[0]["ligand"].x[:, 0], 0).cpu().numpy()
                )
                ...
                if args.gnina_minimize:
                    print(
                        "Running gnina on all predicted ligand positions for energy minimization."
                    )
                    ...
                    conf = confidence
                    if conf is not None and isinstance(
                        ...
                        for i in range(len(orig_ligand_pos)):
                            try:
                                rmsd = get_symmetry_rmsd(
                                    mol, orig_ligand_pos[i], gnina_ligand_pos, gnina_mol
                                )
                            except Exception as e:
                                print(
                                    "Using non corrected RMSD because of the error:", e
                                )
                                rmsd = np.sqrt(
                                    ((gnina_ligand_pos - orig_ligand_pos[i]) ** 2)
                                    .sum(axis=1)
                                    .mean(axis=0)
                                )
                            rmsds.append(rmsd)
                    ...
                    assert gnina_rmsds.shape == (args.gnina_poses_to_optimize,), (
                        str(gnina_rmsds.shape) + " " + str(args.gnina_poses_to_optimize)
                    )
                ...
                for i in range(len(orig_ligand_pos)):
                    try:
                        rmsd = get_symmetry_rmsd(
                            mol, orig_ligand_pos[i], [l for l in ligand_pos]
                        )
                    except Exception as e:
                        print("Using non corrected RMSD because of the error:", e)
                        rmsd = np.sqrt(
                            ((ligand_pos - orig_ligand_pos[i]) ** 2)
                            .sum(axis=2)
                            .mean(axis=1)
                        )
                    rmsds.append(rmsd)
                ...
                centroid_distance = np.min(
                    np.linalg.norm(
                        ligand_pos.mean(axis=1)[None, :]
                        - orig_ligand_pos.mean(axis=1)[:, None],
                        axis=2,
                    ),
                    axis=0,
                )
                ...
                if confidence is not None:
                    ...
                    print(
                        ...
                        (
                            (" gnina rmsd " + str(np.around(gnina_rmsds, 1)))
                            if args.gnina_minimize
                            else ""
                        ),
                    )
    ...
    for overlap in ["", "no_overlap_"]:
        if "no_overlap_" == overlap:
            ...
            gnina_rmsds = (
                np.array(gnina_rmsds_list)[without_rec_overlap]
                if args.gnina_minimize
                else None
            )
            gnina_score = (
                np.array(gnina_score_list)[without_rec_overlap]
                if args.gnina_minimize
                else None
            )
        ...
        performance_metrics.update(
            {
                ...
                f"{overlap}min_rmsds_below_2": (
                    100 * (np.min(rmsds, axis=1) < 2).sum() / len(rmsds)
                ),
                f"{overlap}min_rmsds_below_5": (
                    100 * (np.min(rmsds, axis=1) < 5).sum() / len(rmsds)
                ),
        ...
        if args.gnina_minimize:
            ...
            filtered_rmsds_gnina = gnina_rmsds[
                np.arange(gnina_rmsds.shape[0])[:, None], score_ordering
            ][:, 0]

            performance_metrics.update(
                {
                    f"{overlap}gnina_rmsds_below_2": (
                        (
                            100
                            * (gnina_rmsds < 2).sum()
                            / len(gnina_rmsds)
                            / args.gnina_poses_to_optimize
                        )
                        if args.gnina_minimize
                        else None
                    ),
                    f"{overlap}gnina_rmsds_below_5": (
                        (
                            100
                            * (gnina_rmsds < 5).sum()
                            / len(gnina_rmsds)
                    ...
                    f"{overlap}gnina_min_rmsds_below_2": (
                        (
                            100
                            * (np.min(gnina_rmsds, axis=1) < 2).sum()
                            / len(gnina_rmsds)
                        )
                        if args.gnina_minimize
                        else None
                    ),
                    f"{overlap}gnina_min_rmsds_below_5": (
                        (
                            100
                            * (np.min(gnina_rmsds, axis=1) < 5).sum()
                            / len(gnina_rmsds)
                        )
                        if args.gnina_minimize
                        else None
                    ),
                    f"{overlap}gnina_filtered_rmsds_below_2": (
                        100
                        * (filtered_rmsds_gnina < 2).sum()
                        / len(filtered_rmsds_gnina)
                    ).__round__(2),
                    f"{overlap}gnina_filtered_rmsds_below_5": (
                        100
                        * (filtered_rmsds_gnina < 5).sum()
                        / len(filtered_rmsds_gnina)
                    ).__round__(2),
                    f"{overlap}gnina_rmsds_percentile_25": np.percentile(
                        gnina_rmsds, 25
                    ).round(2),
                    f"{overlap}gnina_rmsds_percentile_50": np.percentile(
                        gnina_rmsds, 50
                    ).round(2),
                    f"{overlap}gnina_rmsds_percentile_75": np.percentile(
                        gnina_rmsds, 75
                    ).round(2),
                }
            )
    ...
    if args.wandb:
        ...
        if confidence_model is not None:
            histogram_metrics_list.append(
                ("reverse_filtered_rmsds", reverse_filtered_rmsds)
            )
            histogram_metrics_list.append(
                (
                    "reverse_filtered_centroid_distances",
                    reverse_filtered_centroid_distances,
                )
            )
            ...
            if N >= 5:
                histogram_metrics_list.append(
                    ("top5_filtered_rmsds", top5_filtered_rmsds)
                )
                histogram_metrics_list.append(
                    (
                ...
                histogram_metrics_list.append(
                    ("top5_reverse_filtered_rmsds", top5_reverse_filtered_rmsds)
                )
                histogram_metrics_list.append(
                    (
            ...
            if N >= 10:
                ...
                histogram_metrics_list.append(
                    ("top10_reverse_filtered_rmsds", top10_reverse_filtered_rmsds)
                )
                histogram_metrics_list.append(
                    (
