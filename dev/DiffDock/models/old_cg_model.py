    ...
    def forward(self, data):
        ...
        if self.include_miscellaneous_atoms:
            ...
            (
                ...
                atom_edge_sh,
                atom_edge_weight,
            ) = self.build_misc_atom_conv_graph(data)
        ...
        for l in range(len(self.lig_conv_layers)):
            ...
            if self.include_miscellaneous_atoms:
                ...
                la_update = self.la_conv_layers[l](
                    atom_node_attr,
                    la_edge_index,
                    la_edge_attr_,
                    la_edge_sh,
                    out_nodes=lig_node_attr.shape[0],
                    edge_weight=la_edge_weight,
                )
            ...
            if l != len(self.lig_conv_layers) - 1:
                ...
                rl_update = self.lig_to_rec_conv_layers[l](
                    lig_node_attr,
                ...
                if self.include_miscellaneous_atoms:
                    ...
                    atom_update = self.misc_conv_layers[l](
                        atom_node_attr,
                        atom_edge_index,
                        atom_edge_attr_,
                        atom_edge_sh,
                        edge_weight=atom_edge_weight,
                    )
                    ...
                    al_update = self.al_conv_layers[l](
                        lig_node_attr,
                    ...
                    ar_update = self.ar_conv_layers[l](
                        rec_node_attr,
                        ar_edge_index,
                        ar_edge_attr_,
                        ar_edge_sh,
                        out_nodes=atom_node_attr.shape[0],
                        edge_weight=ar_edge_weight,
                    )
                    ...
                    ra_update = self.ra_conv_layers[l](
                        atom_node_attr,
            ...
            if l != len(self.lig_conv_layers) - 1:
                ...
                if self.include_miscellaneous_atoms:
                    ...
                    atom_node_attr = F.pad(
                        atom_node_attr,
                        (0, atom_update.shape[-1] - atom_node_attr.shape[-1]),
                    )
                    atom_node_attr = (
                        atom_node_attr + atom_update + al_update + ar_update
                    )
