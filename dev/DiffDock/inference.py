...
def get_parser():
    ...
    parser.add_argument(
        "--config", type=FileType(mode="r"), default="default_inference_args.yaml"
    )
    parser.add_argument(
        "--protein_ligand_csv",
        type=str,
...
def main(args):
    ...
    if not os.path.exists(args.model_dir):
        ...
        for remote_url in remote_urls:
            try:
                ...
                files_downloaded = download_and_extract(
                    remote_url, os.path.dirname(args.model_dir)
                )
                if not files_downloaded:
                    logger.info(f"Download from {remote_url} failed.")
                    continue
                logger.info(
                    f"Downloaded and extracted {len(files_downloaded)} files from {remote_url}"
                )
    ...
    tr_schedule = get_t_schedule(
        inference_steps=args.inference_steps, sigma_schedule="expbeta"
    )
    ...
    for idx, orig_complex_graph in tqdm(enumerate(test_loader)):
        if not orig_complex_graph.success[0]:
            skipped += 1
            logger.warning(
                f"The test dataset did not contain {test_dataset.complex_names[idx]} for {test_dataset.ligand_descriptions[idx]} and {test_dataset.protein_files[idx]}. We are skipping this complex."
            )
            continue
        try:
