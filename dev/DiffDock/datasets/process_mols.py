...
from datasets.parse_chi import (
    get_chi_angles,
    get_coords,
    aa_idx2aa_short,
    get_onehot_sequence,
...
def moad_extract_receptor_structure(
    path,
    complex_graph,
    neighbor_cutoff=20,
    max_neighbors=None,
...
def new_extract_receptor_structure(
    seq,
    all_coords,
    complex_graph,
    neighbor_cutoff=20,
    ...
    n_rel_pos, c_rel_pos = (
        ...
        all_coords[:, 2, :] - all_coords[:, 1, :],
    )
    side_chain_vecs = torch.from_numpy(
        np.concatenate([chi_angles / 360, n_rel_pos, c_rel_pos], axis=1)
    )
    ...
    res_names_list = [
        aa_short2long[seq[i]] if seq[i] in aa_short2long else "misc"
        for i in range(len(seq))
    ]
    feature_list = [
        [safe_index(allowable_features["possible_amino_acids"], res)]
        for res in res_names_list
    ]
    node_feat = torch.tensor(feature_list, dtype=torch.float32)

    lm_embeddings = (
        torch.tensor(np.concatenate(lm_embeddings, axis=0))
    ...
    if all_atoms:
        ...
        atom_coords = torch.from_numpy(
            atom_coords[~np.any(np.isnan(atom_coords), axis=1)]
        ).float()

        if knn_only_graph:
            atoms_edge_index = knn_graph(
                atom_coords, k=atom_max_neighbors if atom_max_neighbors else 1000
            )
        else:
            ...
            for i in range(len(atom_coords)):
                ...
                if len(dst) == 0:
                    dst = list(np.argsort(atoms_distances[i, :]))[
                        1:2
                    ]  # choose second because first is i itself
                    print(
            ...
            atoms_edge_index = torch.from_numpy(
                np.asarray([atom_dst_list, atom_src_list])
            )
        ...
        c_alpha_idx = np.concatenate(
            [np.zeros(len(f)) + i for i, f in enumerate(feats)]
        )
...
def get_moad_atom_feats(res, coords):
    ...
    for i, c in enumerate(coords):
        ...
        if res == "-":
            ...
            atom_feats.append(
                safe_index(allowable_features["possible_amino_acids"], res_long)
            )
            if i >= len(res_order):
                atom_feats.extend(
                    [
                ...
                atom_feats.extend(
                    [
                        safe_index(
                            allowable_features["possible_atomic_num_list"], atomic_num
                        ),
                        safe_index(
                            allowable_features["possible_atom_type_2"],
                            (atom_name + "*")[:2],
                        ...
                        safe_index(
                            allowable_features["possible_atom_type_3"], atom_name
                        ),
                    ]
                )
    ...
    if matching:
        ...
        if keep_original:
            ...
            complex_graph["ligand"].orig_pos = (
                np.asarray(positions) if len(positions) > 1 else positions[0]
            )
