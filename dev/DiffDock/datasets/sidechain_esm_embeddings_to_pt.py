...
parser.add_argument(
    "--esm_embeddings_path",
    type=str,
    default="data/BindingMOAD_2020_ab_processed_biounit/moad_sequences_new",
    help="",
)
parser.add_argument(
    "--output_path",
    type=str,
    default="data/BindingMOAD_2020_ab_processed_biounit/moad_sequences_new.pt",
    help="",
)
...
available = set(
    [filename.split(".")[0] for filename in os.listdir(args.esm_embeddings_path)]
)
final = available.intersection(ids)

for idp in tqdm(final):
    dic[idp] = torch.load(os.path.join(args.esm_embeddings_path, idp + ".pt"))[
        "representations"
    ][33]
