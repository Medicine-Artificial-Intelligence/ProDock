...
def get_coords(prody_pdb):
    ...
    for i, resind in enumerate(resindices):
        ...
        for j, name in enumerate(
            atom_order[aa_long2short[resname] if resname in aa_long2short else "X"]
        ):
...
def get_onehot_sequence(seq):
    ...
    for i, aa in enumerate(seq):
        idx = (
            aa_short2aa_idx[aa] if aa in aa_short2aa_idx else 7
        )  # 7 is the index for GLY
        onehot[i, idx] = 1
    return onehot
...
def get_dihedral_indices(onehot_sequence):
    return np.array(
        [
            dihedral_indices[aa_idx2aa_short[aa_idx]]
            for aa_idx in np.where(onehot_sequence)[1]
        ]
    )
