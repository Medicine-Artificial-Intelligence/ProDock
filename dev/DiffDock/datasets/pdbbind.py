...
class NoiseTransform(BaseTransform):
    ...
    def apply_noise(
        ...
        if self.time_independent:
            ...
            rmsd = np.sqrt(
                ((ligand_pos - orig_ligand_pos) ** 2).sum(axis=1).mean(axis=0)
            )
...
class PDBBind(Dataset):
    def __init__(
        ...
        self.full_cache_path = os.path.join(
            cache_path,
            f"{dataset}3_limit{self.limit_complexes}"
            ...
            + (
                ""
                if not self.fixed_knn_radius_graph
                else (f"_fixedKNN" if not self.knn_only_graph else "_fixedKNNonly")
            )
        ...
        if self.esm_embeddings_path is not None:
            ...
            for key, embedding in id_to_embeddings.items():
                ...
                if key_name in complex_names_all:
                    ...
                    chain_indices_dictlist[key_name].append(
                        int(key.split("_chain_")[1])
                    )
            ...
            for name in complex_names_all:
                ...
                reordered_chains = [
                    complex_chains_embeddings[i] for i in chain_reorder_idx
                ]
    ...
    def collect_all_complexes(self):
        ...
        if os.path.exists(os.path.join(self.full_cache_path, f"heterographs.pkl")):
            with open(
                os.path.join(self.full_cache_path, "heterographs.pkl"), "rb"
            ) as f:
    ...
    def get_complex(self, par):
        ...
        try:
            ...
            lig = read_mol(
                self.pdbbind_dir, name, suffix=self.ligand_file, remove_hs=False
            )
