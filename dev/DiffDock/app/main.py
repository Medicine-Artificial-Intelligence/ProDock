...
def run_wrapper(
    protein_pdb_id, protein_file, ligand_smile, ligand_file, config_file, *args
) -> Tuple[str, Optional[str], Optional[Dict], Optional[gr.Dropdown]]:
    ...
    if output_file:
        ...
        dropdown = gr.Dropdown(
            interactive=True, label="Ranked samples", choices=labels, value=init_value
        )
...
def run():
    ...
    with gr.Blocks(title="DiffDock Web") as demo:
        ...
        gr.Markdown(
            f"""Run [DiffDock]({PROJECT_URL}) for a single protein and ligand.
        ...
        with gr.Box():
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    protein_pdb_id = gr.Textbox(
                        placeholder="PDB Code or upload file below",
                        label="Input PDB ID",
                    )
                    protein_pdb_file = gr.File(
                        file_count="single", label="Input PDB File"
                    )
                with gr.Column():
                    gr.Markdown("## Ligand")
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    config_file = gr.File(
                        label="Configuration (Optional, YML)",
                        file_types=[".yml", ".yaml"],
                        value=None,
        ...
        with gr.Box():
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    view_result_selector = gr.Dropdown(
                        interactive=True, label="Ranked samples"
                    )
                    viewer = gr.HTML(
                        value=init_value, label="Protein Viewer", show_label=True
                    )

        with gr.Row():
            gr.Markdown(
                "Many thanks to [Simon Duerr](https://huggingface.co/simonduerr), who created the "
        ...
        run_btn.click(
            fn=run_wrapper, inputs=_inputs, outputs=_outputs, preprocess=False
        )

        view_result_selector.change(
            fn=update_view,
