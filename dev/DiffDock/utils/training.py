...
def loss_function(
    ...
    tr_loss = ((tr_pred.cpu() - tr_score.cpu()) ** 2 * tr_sigma.cpu() ** 2).mean(
        dim=mean_dims
    )
    ...
    rot_loss = (((rot_pred.cpu() - rot_score.cpu()) / rot_score_norm) ** 2).mean(
        dim=mean_dims
    )
    rot_base_loss = (
        ((rot_score.cpu() / rot_score_norm) ** 2).mean(dim=mean_dims).detach()
    )
    ...
    if backbone_weight > 0:
        backbone_vecs = (
            torch.cat([d["receptor"].side_chain_vecs.cpu() for d in data], dim=0)
            if device.type == "cuda"
            else data["receptor"].side_chain_vecs
        )
        backbone_vecs = backbone_vecs[:, 4:]
        backbone_pred = sidechain_pred[:, 4:]

        backbone_base_loss = (backbone_vecs**2).detach().mean(dim=1) + 0.0001
        backbone_loss = ((backbone_pred.cpu() - backbone_vecs) ** 2).mean(
            dim=1
        ) / backbone_base_loss.mean()
        backbone_base_loss = backbone_base_loss / backbone_base_loss.mean()
        if apply_mean:
            backbone_loss, backbone_base_loss = backbone_loss.mean() * torch.ones(
                1, dtype=torch.float
            ), backbone_base_loss.mean() * torch.ones(1, dtype=torch.float)
        else:
            index = (
                torch.cat(
                    [
                        torch.ones((d["receptor"].pos.shape[0])) * i
                        for i, d in enumerate(data)
                    ],
                    dim=0,
                ).long()
                if device.type == "cuda"
                else data["receptor"].batch
            )
            num_graphs = len(data) if device.type == "cuda" else data.num_graphs
            s_l, s_b_l, c = (
                torch.zeros(num_graphs),
        ...
        if apply_mean:
            backbone_loss, backbone_base_loss = torch.zeros(
                1, dtype=torch.float
    ...
    if sidechain_weight > 0:
        ...
        chi_pred = torch.where(
            torch.isnan(chi_angles),
            torch.zeros_like(chi_angles, device=device),
            chi_pred,
        )
        chi_angles = torch.where(
            torch.isnan(chi_angles),
            torch.zeros_like(chi_angles, device=device),
            chi_angles,
        )
        ...
        difference = torch.min(
            difference, 1 - difference
        )  # angles are circular and 360 degrees = 1
        ...
        if apply_mean:
            (
                sidechain_loss,
                sidechain_base_loss,
            ) = sidechain_loss.mean().cpu() * torch.ones(
                1, dtype=torch.float
            ), sidechain_base_loss.mean().cpu() * torch.ones(
                1, dtype=torch.float
            )
        else:
            index = (
        ...
        if apply_mean:
            sidechain_loss, sidechain_base_loss = torch.zeros(
                1, dtype=torch.float
            ), torch.zeros(1, dtype=torch.float)
        else:
            sidechain_loss, sidechain_base_loss = torch.zeros(
    ...
    loss = (
        ...
        + rot_loss * rot_weight
        + tor_loss * tor_weight
        + sidechain_loss * sidechain_weight
        + backbone_loss * backbone_weight
    )
    return (
        loss,
        tr_loss.detach(),
        rot_loss.detach(),
        tor_loss.detach(),
        backbone_loss.detach(),
        sidechain_loss.detach(),
        tr_base_loss,
        rot_base_loss,
        tor_base_loss,
        backbone_base_loss,
        ...
        if self.intervals == 1:
            ...
            for type_idx, v in enumerate(vals):
                self.acc[self.types[type_idx]] += (
                    v.sum().cpu() if self.unpooled_metrics else v.cpu()
                )
        else:
            for type_idx, v in enumerate(vals):
        ...
        try:
            ...
            if test_sigma_intervals > 0:
                complex_t_tr, complex_t_rot, complex_t_tor = [
                    torch.cat([data[i].complex_t[noise_type] for i in range(len(data))])
                    for noise_type in ["tr", "rot", "tor"]
                ]
                ...
                meter_all.add(
                    [loss_tuple[0].cpu().detach(), *loss_tuple[1:]],
                    [
                        sigma_index_tr,
                        sigma_index_tr,
                        sigma_index_rot,
...
def inference_epoch_fix(model, complex_graphs, device, t_to_sigma, args):
    ...
    for orig_complex_graph in tqdm(loader):
        data_list = [
            copy.deepcopy(orig_complex_graph) for _ in range(args.inference_samples)
        ]
        ...
        try:
            ...
            print(
                "problem with orig_pos which is of shape:",
                orig_complex_graph["ligand"].orig_pos.shape,
                e,
            )
            continue
        ...
        for i in range(len(orig_ligand_pos)):
            try:
                ...
                rmsd = np.sqrt(
                    ((ligand_pos - orig_ligand_pos[i]) ** 2).sum(axis=2).mean(axis=1)
                )
